...
- **Extensible**
  ...
  [Add Your Database](#adding-your-own-database)

- **Backward compatible**

  MySQL DDL is unchanged — existing deployments need no migration.

  [Compatibility Notes](#backward-compatibility)

Prerequisites

See [Implementation](https://vadikko2.github.io/python-cqrs-mkdocs/outbox/implementation/index.md) for the repository interface and the outbox table structure, and [Usage](https://vadikko2.github.io/python-cqrs-mkdocs/outbox/usage/index.md) for event registration and publishing.
...
The type layer is shared by the whole library, not just the outbox. `JSONType` renders as plain `JSON` on every dialect and exists so that a project can opt into PostgreSQL `JSONB` with one registration — see [Saga Storage](https://vadikko2.github.io/python-cqrs-mkdocs/saga/storage/#optional-jsonb-on-postgresql). Everything below about handlers, placement and registration order applies to it unchanged.
...
1. **`VARBINARY` is variable-length** for a value that is always exactly 16 bytes. `BINARY(16)` is fixed-width and a better key for `event_id_unique_index`.
...
- [Implementation](https://vadikko2.github.io/python-cqrs-mkdocs/outbox/implementation/index.md) — Repository interface and table structure
- [Best Practices](https://vadikko2.github.io/python-cqrs-mkdocs/outbox/best_practices/index.md) — Recommendations for reliable event delivery
- [Saga Storage](https://vadikko2.github.io/python-cqrs-mkdocs/saga/storage/index.md) — Persistence for the Saga pattern
