...
!!! info "Stored values are the member names"
    The column is mapped with `sqlalchemy.Enum(EventStatus)`, which persists the **names** of the enum members.
    In the database you will see `NEW`, `PRODUCED` and `NOT_PRODUCED` — not the lowercase Python values. This
    matters when you query the table with raw SQL or write migrations by hand.
...
The outbox model is built from dialect-aware column types, so the rendered DDL differs between databases. The
two supported variants are:
...
!!! tip "Other databases and Alembic"
    See [Database Support](databases.md) for the full support matrix, the type mapping per dialect, the Alembic
    recipe and how to register a native type for a database the library does not know about.
...
By default, the table name is `outbox`. To rename it — or to attach the outbox model to your own declarative
base so that it shares one `MetaData` with the rest of your schema — use `rebind_outbox_model`:
...
!!! warning "Call it before the table is used"
    `rebind_outbox_model` mutates the model and re-registers its table in the target metadata, so call it at
    import/startup time — before `create_all()`, before generating migrations and before the first query.
