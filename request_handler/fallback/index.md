...
- **Back to Request Handlers Overview**

  Return to the Request Handlers overview page.

  [Back to Overview](https://vadikko2.github.io/python-cqrs-mkdocs/request_handler/index.md)
...
There is no `fallback_shares_scope` flag — use SEND to share the transaction, HANDLER for a clean alternative path. See [Scope Strategies](https://vadikko2.github.io/python-cqrs-mkdocs/scoped_dependencies/strategies/index.md).
...
After `fail_max` failures, the circuit opens: the primary handler is not called and the fallback runs immediately. See [Saga Fallback — Circuit Breaker](https://vadikko2.github.io/python-cqrs-mkdocs/saga/fallback/circuit_breaker/index.md) for the same three-state pattern (CLOSED / OPEN / HALF_OPEN).
...
**Storage:** By default the circuit state is in-memory (per process). For distributed apps, you can pass a `storage_factory` that returns Redis (or other) storage so the circuit state is shared across instances. See [Saga Fallback — Circuit Breaker: Storage Configuration](https://vadikko2.github.io/python-cqrs-mkdocs/saga/fallback/circuit_breaker/#storage-configuration-memory-vs-redis).

**Failure filtering:** Use `failure_exceptions` on `RequestHandlerFallback` to restrict which exceptions trigger fallback; use `exclude` on `AioBreakerAdapter` to prevent certain exceptions from opening the circuit. Details: [Saga Fallback — Circuit Breaker: Failure Exception Filtering and Business Exception Exclusion](https://vadikko2.github.io/python-cqrs-mkdocs/saga/fallback/circuit_breaker/#failure-exception-filtering).

## Streaming Handlers

`RequestHandlerFallback` also supports **streaming handlers**. Both `primary` and `fallback` can be `StreamingRequestHandler`. When the primary stream raises (e.g. after yielding some items), the dispatcher switches to the fallback handler's stream. See [Stream Handling Fallback](https://vadikko2.github.io/python-cqrs-mkdocs/stream_handling/fallback/index.md) for details.

## Related

- [Event Handler Fallback](https://vadikko2.github.io/python-cqrs-mkdocs/event_handler/fallback/index.md) — Fallback for event handlers
- [Stream Handling Fallback](https://vadikko2.github.io/python-cqrs-mkdocs/stream_handling/fallback/index.md) — Fallback for streaming handlers
- [Chain of Responsibility](https://vadikko2.github.io/python-cqrs-mkdocs/chain_of_responsibility/index.md) — Can be combined with fallback (COR as primary)
