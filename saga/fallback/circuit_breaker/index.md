...
1. Primary step executes normally
1. Failures are counted
1. After `fail_max` failures → transition to OPEN
1. **OPEN** (Fail Fast)
1. Primary step is **NOT executed** (fail fast)
1. `CircuitBreakerError` is raised immediately
1. Fallback executes automatically
1. After `timeout_duration` → transition to HALF_OPEN
1. **HALF_OPEN** (Testing Recovery)
1. Single request allowed (test if service recovered)
1. Success → transition to CLOSED
1. Failure → transition back to OPEN
