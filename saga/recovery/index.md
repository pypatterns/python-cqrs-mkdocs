...
1. Periodically scanning for incomplete sagas (via `get_sagas_for_recovery`)
1. Resuming execution from the last completed step
1. Completing compensation if saga was in compensating state
1. Tracking **recovery attempts** — on recovery failure, the storage increments `recovery_attempts` automatically so sagas can be retried or excluded when the limit is reached
...
1. Reconstruct context object from persisted data
1. Resume execution from last completed step or complete compensation
...
1. For SQL databases, this executes `SELECT ... FOR UPDATE`.
1. This acquires a database-level lock on the saga row.
1. If another worker tries to recover the same saga, it will block until the first worker completes or releases the lock.
...
1. **Use `max_recovery_attempts`** — Exclude sagas that fail recovery too many times (e.g. 5) to avoid infinite retries
1. **Use `stale_after_seconds`** — Avoid picking sagas that are currently being executed by another worker
1. **Use `saga_name` for per-type recovery** — When running separate recovery jobs per saga type, pass `saga_name` so each job only processes its own sagas
1. **Match `scope_strategy`** — Pass the same strategy, `container`, and `storage` as the original run. A plain container is enough; `SagaTransaction` wraps it internally
1. **Do not recover from an open SEND request** — `recover_saga(SEND)` inside `send()` joins that request UoW (`reuse_existing=True`). Run recovery from a worker
1. **Handle failures** — Log errors and send alerts; `increment_recovery_attempts` is called automatically by `recover_saga`
1. **Monitor metrics** — Track recovery rate, duration, failures, and sagas exceeding max attempts
1. **Use persistent storage** — Memory storage loses data on restart
